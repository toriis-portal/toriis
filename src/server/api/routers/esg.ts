...
const consumeExternalApi = async <T>(url: string): Promise<T | errorMessage> => {
...
export const esgRouter = createTRPCRouter({
  esgpull: publicProcedure.input(z.string()).query(async ({ input, ctx }) => {
    ...
    companies.map( async (company: { ticker: string; id: string }, index: number) => {
      if (company.ticker !== 'NO_TICKER_FOUND') {
        const url = `https://esgapiservice.com/api/authorization/search?q=${company.ticker}&token=${process.env.ESG_API_KEY ?? ''}`
      
        const api_res = await consumeExternalApi<ESG[]>(url)
        console.log(api_res)

        if ('error' in api_res || 'message' in api_res || !api_res) {
          console.log(api_res)
          ...
          const ESGData = (api_res as ESG[])[0]
